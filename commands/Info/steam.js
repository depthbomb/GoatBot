...
exports.run = async (client, message, args, level) => {
	...
	if (action === "validate" || action === "valid") {
		...
		request(`https://steamcommunity.com/profiles/${communityId}`, (err, res, body) => {
			if (err) {
				msg.delete();
				return client.error(message, err);
			}

			const $ = cheerio.load(body);	//	Load the profile page so we can get info the API doesn't provide
			const description = $('meta[name=Description]').attr('content');	//	Get the profile description from the page's meta tag since it has a cleaned version
			request({
				headers: {
